...
inline void arange_check_bounds(
    ...
    if (isComplexType(start.type()) || isComplexType(end.type()) || isComplexType(step.type())) {
      ...
      TORCH_CHECK((stepc.real() == 0 && stepc.imag() != 0) 
          || (stepc.real() != 0 && stepc.imag() == 0), 
          "at least one of the parts of complex step must be nonzero");
...
int64_t compute_arange_size(const Scalar& start, const Scalar& end, const Scalar& step) {
  ...
  if constexpr (std::is_same_v<scalar_t, int64_t>) {
    ...
    step_t xstartc = start.to<step_t>();
    step_t xendc = end.to<step_t>();
    step_t xstepc = step.to<step_t>();
    auto dist = xendc - xstartc;
    TORCH_CHECK_VALUE((xstepc.real() != 0 || xstepc.imag() != 0), "step must be nonzero");
    if(xstepc.real() == 0) {
      int64_t sgn = (xstepc.imag() > 0) - (xstepc.imag() < 0);
      size_d = std::ceil((dist.imag() + xstepc.imag() - sgn) / xstepc.imag());
    } else {
      int64_t sgn = (xstepc.real() > 0) - (xstepc.real() < 0);
      size_d = std::ceil((dist.real() + xstepc.real() - sgn) / xstepc.real());
    }
  } else {
